...
INSERT OR REPLACE INTO bug_reports (id, app_id, reporter_name, title, description, severity, status) VALUES
  ('bug_001',
   'app_jsdiff_demo',
   'Alice (tester)',
   'word-diff treats trailing whitespace as a difference',
   'When diffing two strings that only differ by trailing whitespace, word-diff returns a non-empty changeset. Repro file: src/diff/word.ts. Expected: empty diff for whitespace-equivalent input.',
   'medium',
   'open'),
  ('bug_002',
   'app_jsdiff_demo',
